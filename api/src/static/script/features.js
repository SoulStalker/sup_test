document.addEventListener('DOMContentLoaded', function () {
    ...
    if (openModalButton) {
        openModalButton.addEventListener('click', function () {
            ...
            form.setAttribute('action', '/projects/features/create/');
            modalTitle.textContent = 'Создать фичу';
            submitButton.textContent = 'Создать фичу';
            form.reset();
            clearErrors();
    ...
    if (form) {
        form.addEventListener('submit', function (event) {
            ...
            let url = form.action;
            if (currentProjectId) {
                url = `/projects/features/edit/${currentProjectId}/`;
            }
            fetch(url, {
                method: 'POST',
                body: formData,
            ...
            .then(data => {
                if (data.status === 'success') {
                    closeModal();
                    return fetch('/projects/features/', {
                        method: 'GET',
                        headers: {
                            'X-Requested-With': 'XMLHttpRequest',
                            'Accept': 'text/html'
    ...
    function handleEditClick() {
        const projectId = this.getAttribute('data-feature-id');
        if (!projectId) {
            console.error('ID проекта не найден');
            return;
        }

        fetch(`/projects/features/edit/${projectId}/`, {
            headers: {
                'X-Requested-With': 'XMLHttpRequest'
            }
        })
    ...
    if (confirmDeleteButton) {
        confirmDeleteButton.addEventListener('click', function () {
            ...
            fetch(`/projects/features/delete/${currentProjectId}/`, {
                method: 'DELETE',
                headers: {
    ...
    modalTitle.textContent = 'Редактирование фичи';
    form.setAttribute('action', `/projects/features/edit/${data.id}/`);
    ...
    if (participantsSelect && data.participants) {
        ...
        console.log('Participant IDs:', participantIds); // Логируем ID участников
    ...
    function initializeTableEventHandlers() {
        ...
        deleteButtons.forEach(button => {
            button.addEventListener('click', function() {
                currentProjectId = this.getAttribute('data-feature-id');
